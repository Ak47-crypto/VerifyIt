...
function ManuFacturerLogin() {
  const { host, alerts, handleAlerts, connection, setConnection, contractAdd } =
    useContext(context);
  ...
  const onImageCownload = () => {
    ...
    img.onload = () => {
      ...
      downloadLink.download = `${
        qrName ? qrName : blockData[1] + blockData[2].replace(/\s/g, "")
      }`;
    ...
    if (window.ethereum) {
      if (window.ethereum.networkVersion !== 11155111) {
        try {
          ...
          try {
            spinner.blockConfirm = true;
            setSpinner({ ...spinner });
            const provider = new ethers.BrowserProvider(window.ethereum);
            const signer = await provider.getSigner(walletAddress);
            const setProduct = new ethers.Contract(
              contractAdd,
              abi.abi,
              signer
            );
            const transaction = await setProduct.addProduct(
              parseInt(data.sn),
              data.productName,
              data.source,
              data.destination
            ...
            handleAlerts(
              "Please wait for the Network confirmation, Do not refresh",
              "warning",
              6000
            );

            // checkingTransaction();
            // const checkingTransaction=async()=>{
            try {
              const res = await provider.waitForTransaction(transaction.hash);
              if (res.blockNumber) {
                handleAlerts(
                  `Block created at ${res.blockNumber},Transaction successfull`,
                  "success",
                  6000
                );
              } else {
                handleAlerts(`Transaction Still Pending`, "warning", 6000);
                // checkingTransaction();
              }
            } catch (err) {
            ...
            if (
              ...
              String(err.value) === "NaN"
            ) {
              handleAlerts(
                `${err.code}: Serial Number must be a Number`,
                "warning"
              );
            } else handleAlerts(`${err.reason}`, "warning");
          }
        } catch (err) {
          handleAlerts(`${err.message}`, "warning");
        }
      } else handleAlerts("sepolia network not added", "danger");
    } else {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      handleAlerts(
        `${err.message == "Failed to fetch" ? "Server Down" : err.message}`,
        "danger"
      );
    }
    spinner.sign = false;
    ...
    return (
      <>
        {spinner.load ? (
          ...
          <div>
            <div
              className="d-flex flex-column justify-content-center my-3 mx-3 opacity75"
              style={{ minHeight: "calc(100vh - 152px)", opacity: ".8" }}
            >
              <div className="row">
                <div className="col-md-8 p-5">
                  <div className="card" style={infoStyle.box}>
                    ...
                    <div className="card-body">
                      <h5 className="card-title" style={infoStyle.h.sizes.h1}>
                        Company name : {fetchData && fetchData.name}
                      </h5>
                      <div>
                        <p className="card-text">
                          Address : {fetchData && fetchData.address}
                        </p>
                        <p href="#">
                          Company Email : {fetchData && fetchData.email}
                        </p>
                    ...
                    <div className="container">
                      <form onSubmit={providerWalletGet}>
                        <div className=" d-flex flex-column justify-content-center">
                          <label
                            style={infoStyle.h.sizes.label}
                            className="form-label text-center"
                          >
                            Enter Serial Number
                          </label>
                          <input
                            style={infoStyle.widths}
                            required
                            name="productName"
