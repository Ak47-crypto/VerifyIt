...
function ConsumerLogin() {
  ...
  const providerManufacturerAdd = async (event) => {
    ...
    if (typeof window.ethereum !== "undefined") {
      try {
        ...
        const addManufacturer = new ethers.Contract(
          contractAdd,
          abi.abi,
          signer
        );
  ...
  return (
    <div className="mt-5" style={{ minHeight: "calc(100vh - 180px)" }}>
      ...
      >
        {walletAddress ? (
          <h1 style={{ margin: "auto" }}>{walletAddress}</h1>
        ) : (
          <button
            className="btn btn-primary"
            style={{ margin: "auto" }} /*onClick={connectWallet}*/
          >
            Connect Wallet
          </button>
        )}
      ...
      {walletAddress ? (
        <h1 className="mt-3 text-center">Product Flow</h1>
      ) : (
        <div></div>
      )}
      <div
        className="row d-flex justify-content-center align-iteams-center flex-column mx-3"
        style={{ margin: "auto" }}
      >
        {productData &&
          productData.map((product, index) => (
            <div
              ...
              style={{  width: "50rem",backgroundColor:"skyblue",opacity:".9" ,infoStyle }}
            >
              <div className="card-body">
                <h5 className="card-title">
                  SNO:<span className="text-muted">{product[0]}</span>
                </h5>
                <h5 className="card-subtitle mb-2">
                  Product Name: <span className="text-muted">{product[1]}</span>
                </h5>
                <h5 className="card-subtitle mb-2">
                  Source:<span className="text-muted">{product[2]}</span>
                </h5>
                <h5 className="card-subtitle mb-2">
                  Destination:<span className="text-muted">{product[3]}</span>
                </h5>
                <h5 className="card-subtitle mb-2">
                  TimeStamp:<span className="text-muted">{String(timeconverter(product[4]))}</span>
                </h5>
              </div>
            </div>
          ))}
